...
let regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  let rec helper re =
    ...
    | Empty_String ->
        let start = fresh () in
        let accept = fresh () in
        { sigma = []; qs = [start; accept]; q0 = start; fs = [accept]; delta = [(start, None, accept)] }
    | Char c ->
        let start = fresh () in
        let accept = fresh () in
        { sigma = [c]; qs = [start; accept]; q0 = start; fs = [accept]; delta = [(start, Some c, accept)] }
    | Union (r1, r2) ->
        ...
        { sigma = nfa1.sigma @ nfa2.sigma;
          ...
          q0 = start;
          fs = [accept];
          delta = (start, None, nfa1.q0) :: (start, None, nfa2.q0) ::
                 nfa1.delta @ nfa2.delta @ 
                 [(List.hd nfa1.fs, None, accept); (List.hd nfa2.fs, None, accept)]
        }
    | Concat (r1, r2) ->
        let nfa1 = helper r1 in
        let nfa2 = helper r2 in
    ...
    | Star r ->
        ...
        { sigma = nfa.sigma;
          ...
          q0 = start;
          fs = [accept];
          delta = (start, None, nfa.q0) :: (List.hd nfa.fs, None, nfa.q0) :: 
                 nfa.delta @ [(List.hd nfa.fs, None, accept); (start, None, accept)]
        }
  in
  helper regexp
